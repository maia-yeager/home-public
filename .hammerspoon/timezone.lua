...
local function timestampSubmenu(formatter, nowFormatter)
	...
	local function copyTimestamp(datePartial)
		return function()
			hs.pasteboard.writeObjects(os.date(formatter, os.time(mergeDates(os.date("*t"), datePartial))))
		end
	end
...
local function getNextDSTEvent(dtObj)
	...
	return getNextDSTEvent(mergeDates(normalizedDt, { year = dtObj.year + 1, month = 1, day = 1 }))
end
