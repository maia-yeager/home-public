...
# Shrink video for Discord.
function discord-video {
    file=$1
    if [ -z "$file" ]; then
        echo discord-video: must supply an input file.
        return
    fi
    target_size_mb=50  # target size in MB
...
[ -n "$TMUX" ] && setopt ignore_eof # ignore EOF if in tmux
